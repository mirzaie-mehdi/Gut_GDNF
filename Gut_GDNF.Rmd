---
title: "Gut_GDNF"
output: html_document
date: "2023-08-23"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Read Data

In this project we use two single cell RNA-seq data sets which can be downloaded from
For human:
https://www.gutcellatlas.org/

For mice:
https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE149524

GSE149524



## library
```{r library}
library(ggplot2)
library(dplyr)
library(reshape2)
library(ggpubr)
library(tidyverse)
#library(crayon)
```

## Read data

The data sets 
```{r read_data, echo=FALSE}
mice_e15 <- readRDS("data/rds/normExp_MiceE15.rds")
human_sec_trim <- readRDS("data/rds/normExp_HumanSecondTrim.rds")
#human_first_trim <- readRDS("data/rds/human_first.rds")
```

## function for pairwise correlation
## save scatter plot of pair genes

```{r}
mycor <- function(species='human',branch='AB',includeZeros=TRUE){
  species <- str_to_lower(species)
  branch <- str_to_upper(branch)
  if(species=='human'){
    df <- human_sec_trim
  } else if (species=='mice'){
    df <- mice_e15
  }
  if(branch!='AB'){
    b <- paste0('Branch ', branch)
    df <- df[df$Branch==b, ]
    if(species=='human'){
      asub <- unique(df$branch)
      sub <- c(factor('All'),asub)
      sub <- paste0(1:length(sub),': ',sub,'\n')
      cat(green("Note: there are sub branches = \n"),
          green(bold(sub)))
      sb <- readline(prompt = "Please enter number of sub branch = ")
      if(sb!=1){
        sb<-asub[as.numeric(sb)]
        df <- df[df$branch==sb, -c(13,14)]
      } else if(sb==1){
        sb <- b
        df <- df[, -c(13,14)]
        }
    } else if(species=='mice'){
      cat(green("Note: there are not information for sub branch in mice"),'\n')
      sb <- b
      df <- df[, -c(13,14)]
      }
    } else if(branch=='AB'){
      df <- df[, -c(13,14)]
      }
  ######################  initializing
  num_genes <- ncol(df)
  gene_names <- colnames(df)
  p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
  cor_matrix <- p_value_matrix
  ###############################
  mainDir <- getwd()
  subDir <- paste0(species,'_',sb)
  pTOs <- file.path(mainDir, subDir)
  if (!file.exists(pTOs)){
    dir.create(pTOs)
  }
  #############################
  for (i in 1:num_genes) {
    gene1 = gene_names[i]
    for (j in 1:num_genes) {
      x <- df[,i]
      y <- df[,j]
      gene2 = gene_names[j]
      if(!includeZeros){
        non_zero_indices <- which(x != 0 & y != 0)
        x <- x[non_zero_indices]
        y <- y[non_zero_indices]
      }
      dfxy = data.frame(x=x, y=y)
      if (i>j & length(x)>2){
        gp <- ggplot(dfxy, aes(x = x, y = y)) +
          geom_point() +
          ggtitle(paste(subDir,", Number of cells = ",length(x))) +
          #xlim(.01,max(x)) + ylim(.01,max(y)) +
          labs(x = gene1, y = gene2) +
          theme_light(base_line_size = 0) +
          stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
          theme(axis.title = element_text(size = 20,face = 'bold'),
                axis.text = element_text(size = 15,face = 'bold'))
        # Save the plot
        file_name <- paste0(pTOs, '/',  gene1, "_vs_", gene2, ".png")
        ggsave(file_name, gp)
        }
      if (length(x)>2){
        c <- cor.test(x, y,method = "pearson")
        p_value_matrix[i, j] <- c$p.value
        cor_matrix[i, j] <- c$estimate
      }
    }
    }
  rownames(p_value_matrix) <- colnames(p_value_matrix) <- colnames(df)
  rownames(cor_matrix) <- colnames(cor_matrix) <- colnames(df)
  write.csv(p_value_matrix,file = paste0(pTOs, '/','p_value.csv'))
  write.csv(cor_matrix,file = paste0(pTOs, '/','cor_value.csv'))
  cor_obj <- list(pval=p_value_matrix, cor=cor_matrix)
  ################################
  ############# heatmap
  p_value <- p_value_matrix
  p_value[upper.tri(p_value,diag =T)] <- NA
  corr <- cor_matrix
  corr[upper.tri(corr,diag =T)] <- NA
  p_value_r <- na.omit(melt(p_value))
  corr_r <- na.omit(melt(corr))
  p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
  colnames(p_val_corr) <- c("gene1","gene2","P_Value","Corr")
  p_val_corr$L10p <- round(-log10(p_val_corr$P_Value),1)
  gh <- ggplot(p_val_corr)+
    geom_tile(aes(x=gene1,y=gene2, fill = Corr),
              colour = "black", show.legend = TRUE) +
    geom_text(aes(x=gene1,y=gene2,
                  label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
              parse = T,size=6,fontface="bold")+
    scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                         high="#DC0000B2", low="#3C5488B2") +
    theme_test() +
    ggtitle(paste0(subDir,", Number of Cells = ", nrow(df))) +
    theme(axis.title = element_blank(),
          axis.text = element_text( face = "bold",size = 14),
          legend.position = "bottom",
          legend.title = element_text( face = "bold",size = 12),
          legend.text = element_text( face = "bold",size = 10),
          plot.title = element_text(hjust = 0.5))
  # Save the plot
  file_name <- paste0(pTOs, '/',  subDir, ".pdf")
  ggsave(file_name, gh,width = 9,height = 7)
  return(cor_obj)
}
```



## Mice

```{r}
miceA <- mice_e15[mice_e15$Branch=="Branch A",-c(13,14)] # There are 2127 cells.
num_genes = ncol(miceA)
gene_names <- colnames(miceA)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
        x <- miceA[,i]
        y <- miceA[,j]
        gene2 = gene_names[j] 
        non_zero_indices <- which(x != 0 & y != 0)
        x_non_zero <- x[non_zero_indices]
        y_non_zero <- y[non_zero_indices]
        df = data.frame(x=x_non_zero, y=y_non_zero)
        if (i>j){
          sc <- ggplot(df, aes(x = x, y = y)) +
                geom_point() +
            ggtitle(paste("Mice Branch A, Number of cells = ", length(x_non_zero), sep = "")) +
                xlim(.01,max(x_non_zero)) + ylim(.01,max(y_non_zero)) +
                labs(x = gene1,y = gene2) +
                theme_light(base_line_size = 0) +
                stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
                theme(axis.title = element_text(size = 20,face = 'bold'),
                axis.text = element_text(size = 15,face = 'bold'))
                # Save the plot
                file_name <- paste("MiceA_",  gene1, "_vs_", gene2, ".png", sep = "")
                ggsave(file_name, sc)
        }
        if (length(x_non_zero)>2){
           c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
           p_value_matrix[i, j] <- c$p.value
           cor_matrix[i, j] <- c$estimate
        }
#    p_value_matrix[i, j] <- cor.test(miceA[, i], miceA[, j],method = "spearman",exact = FALSE)$p.value
#    cor_matrix[i, j] <- cor.test(miceA[, i], miceA[, j],method = "spearman",exact = FALSE)$estimate
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(miceA)

```

## Visualization_BranchA_mice

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))
p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
g = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2,
                label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Mice_Branch A, Number of Cells = 2127") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
      plot.title = element_text(hjust = 0.5))
#        plot.title = element_text(hjust = 0.5, margin = margin(b = 5), face = "bold",size = 14 ))

```

## Human_Branch A

```{r}
humanA <- human_sec_trim[human_sec_trim$Branch=="Branch A",-c(13,14)] # There are 365 cells.
num_genes = ncol(humanA)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(humanA)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- humanA[,i]
    y <- humanA[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x
    y_non_zero <- y
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch A, Number of cells = ", length(x_non_zero), sep = "")) +
  #    xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("HumanA_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(humanA)
```

## Visualization_BranchA_human

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_Branch A, Number of Cells = 365") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```


## Human_Branch B

```{r}
humanB <- human_sec_trim[human_sec_trim$Branch=="Branch B",-c(13,14)] # There are 512 cells.
num_genes = ncol(humanB)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(humanB)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- humanB[,i]
    y <- humanB[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x
    y_non_zero <- y
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch B, Number of cells = ", length(x_non_zero), sep = "")) +
  #    xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("humanB_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(humanB)
```

## Visualization_BranchB_human

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_Branch B, Number of Cells = 512") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```



## Human A1_Second

```{r}
humanA1 <- human_sec_trim[human_sec_trim$branch=="Branch A1 (iMN)",-c(13,14)] # There are 229 cells.
num_genes = ncol(humanA1)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(humanA1)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- humanA1[,i]
    y <- humanA1[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x[non_zero_indices]
    y_non_zero <- y[non_zero_indices]
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch A1_secondT, Number of cells = ", length(x_non_zero), sep = "")) +
      xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("humanA1_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(humanA1)
```

## Visualization_BranchA1_human_secondT

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_Branch A1_Second, Number of Cells = 229") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```

## First_Trimm_Human

```{r}
first_trim <- readRDS("data/rds/human_first.rds")
human_1t_A <- first_trim[first_trim$branch=="Branch A1 (iMN)",-c(13,14)] # There are 1513 cells.
num_genes = ncol(human_1t_A)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(human_1t_A)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- human_1t_A[,i]
    y <- human_1t_A[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x[non_zero_indices]
    y_non_zero <- y[non_zero_indices]
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch A1_first, Number of cells = ", length(x_non_zero), sep = "")) +
      xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("human_1t_A_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(human_1t_A)

```

## Visualization_FirstTrim_BranchA_human

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_First_Trim_Branch A1, Number of Cells = 1513") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```

## Second_Trimm_Human_A4

```{r}
second_trim <- readRDS("data/rds/normExp_HumanSecondTrim.rds")
human_2t_A4 <- second_trim[second_trim$branch=="Branch A4 (IN)",-c(13,14)] # There are 136 cells.
num_genes = ncol(human_2t_A4)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(human_2t_A4)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- human_2t_A4[,i]
    y <- human_2t_A4[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x[non_zero_indices]
    y_non_zero <- y[non_zero_indices]
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch A1_first, Number of cells = ", length(x_non_zero), sep = "")) +
      xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("human_2t_A4_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(human_2t_A4)

```

## Visualization_FirstTrim_BranchA_human

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_second_trim_Branch A4, Number of Cells = 136") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```

## Human_Branch A_B

```{r}
humanAB <- human_sec_trim[,-c(13,14)]# There are 877 cells.
num_genes = ncol(humanAB)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
gene_names <- colnames(humanAB)
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
    gene2 = gene_names[j] 
    x <- humanAB[,i]
    y <- humanAB[,j]
    non_zero_indices <- which(x != 0 & y != 0)
    x_non_zero <- x
    y_non_zero <- y
    df = data.frame(x=x_non_zero, y=y_non_zero)
    if (i>j){
      sc <- ggplot(df, aes(x = x, y = y)) +
      geom_point() +
      ggtitle(paste("Human Branch A-B, Number of cells = ", length(x_non_zero), sep = "")) +
#      xlim(.01,max(x_non_zero))+ylim(.01,max(y_non_zero)) +
      labs(x = gene1,y = gene2) +
      theme_light(base_line_size = 0) +
      stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
      theme(axis.title = element_text(size = 20,face = 'bold'),
         axis.text = element_text(size = 15,face = 'bold'))
  
    # Save the plot
    file_name <- paste("humanAB_",  gene1, "_vs_", gene2, ".png", sep = "")
    ggsave(file_name, sc)
    }
    if (length(x_non_zero)>2){
       c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
       p_value_matrix[i, j] <- c$p.value
       cor_matrix[i, j] <- c$estimate
    }
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(humanAB)
```

## Visualization_BranchB_human

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))

p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
gh = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Human_Branch A-B, Number of Cells = 877") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
        plot.title = element_text(hjust = 0.5))

```

## Mice_A-B

```{r}
miceAB <- mice_e15[,-c(13,14)] # There are 2476 cells.
num_genes = ncol(miceAB)
gene_names <- colnames(miceAB)
p_value_matrix <- matrix(NA, nrow = num_genes, ncol = num_genes)
cor_matrix = p_value_matrix
for (i in 1:num_genes) {
  gene1 = gene_names[i] 
  for (j in 1:num_genes) {
        x <- miceAB[,i]
        y <- miceAB[,j]
        gene2 = gene_names[j] 
        non_zero_indices <- which(x != 0 & y != 0)
        x_non_zero <- x
        y_non_zero <- y
        df = data.frame(x=x_non_zero, y=y_non_zero)
        if (i>j){
          sc <- ggplot(df, aes(x = x, y = y)) +
                geom_point() +
            ggtitle(paste("Mice Branch A, Number of cells = ", length(x_non_zero), sep = "")) +
 #               xlim(.01,max(x_non_zero)) + ylim(.01,max(y_non_zero)) +
                labs(x = gene1,y = gene2) +
                theme_light(base_line_size = 0) +
                stat_cor(method = "pearson", size=5, color = "red", geom = "label") +
                theme(axis.title = element_text(size = 20,face = 'bold'),
                axis.text = element_text(size = 15,face = 'bold'))
                # Save the plot
                file_name <- paste("miceAB_",  gene1, "_vs_", gene2, ".png", sep = "")
                ggsave(file_name, sc)
        }
        if (length(x_non_zero)>2){
           c <- cor.test(x_non_zero, y_non_zero,method = "pearson")
           p_value_matrix[i, j] <- c$p.value
           cor_matrix[i, j] <- c$estimate
        }
#    p_value_matrix[i, j] <- cor.test(miceAB[, i], miceAB[, j],method = "spearman",exact = FALSE)$p.value
#    cor_matrix[i, j] <- cor.test(miceAB[, i], miceAB[, j],method = "spearman",exact = FALSE)$estimate
  }
}
rownames(p_value_matrix) = rownames(cor_matrix) = colnames(p_value_matrix) = colnames(cor_matrix) = colnames(miceAB)

```

## Visualization_BranchAB_mice

```{r}
p_value <- p_value_matrix
p_value[upper.tri(p_value,diag =T)] <- NA
corr <- cor_matrix
corr[upper.tri(corr,diag =T)] <- NA
p_value_r = na.omit(melt(p_value))
corr_r = na.omit(melt(corr))
p_val_corr <- merge(p_value_r,corr_r,by = c("Var1","Var2"))
colnames(p_val_corr) = c("gene1","gene2","P-Value","Corr")
p_val_corr$L10p = round(-log10(p_val_corr$`P-Value`),1)
g = ggplot(p_val_corr)+
  geom_tile(aes(x=gene1,y=gene2, fill = Corr),colour = "black", show.legend = TRUE) +
  geom_text(aes(x=gene1,y=gene2, label=ifelse(!is.na(L10p),paste0("10^-",L10p),NA)),
            parse = T,size=6,fontface="bold")+
  scale_fill_gradient2(mid="white",limits=c(-1,1),na.value = 'gray80',
                       high="#DC0000B2", low="#3C5488B2") +
 theme_test() + 
  ggtitle("Mice_Branch AB, Number of Cells = 2476") +
  theme(axis.title = element_blank(),
        axis.text = element_text( face = "bold",size = 14),
        legend.position = "bottom",
        legend.title = element_text( face = "bold",size = 12),
        legend.text = element_text( face = "bold",size = 10),
      plot.title = element_text(hjust = 0.5))
#        plot.title = element_text(hjust = 0.5, margin = margin(b = 5), face = "bold",size = 14 ))

```

