...
write.table(out.s, "data/clustering/HumanA_50clustered.txt",
            quote=F, sep="\t")
################################
# summary table for the cluster results
################################
# out.s <- read.delim('data/clustering/HumanA_50clustered.txt')
# dat   <- as.matrix(out.s)
###########
...
# write.table(out.n, "data/clustering/MiceAB_90cluNum.txt", quote=F, sep="\t")
################################
rm(list=setdiff(ls(), sel_var))
```
...
```{r}
out.s <- read.delim('data/clustering/HumanAB_90clustered.txt')
...
#saveRDS(exClu,'data/rds/HumanAB_90exClu.rds')
```

## pairwise correlation 
function for pairwise correlation and save scatter plot of pair genes
...
```{r}
rm(list=setdiff(ls(), 'mycor'))
human_exClu <- readRDS('data/rds/HumanAB_90exClu.rds')
mice_exClu <- readRDS('data/rds/MiceAB_90exClu.rds')

out <- mycor(species='human',branch='AB',includeZeros=TRUE)
# output files will write in the following directory:
#  ./species_Beanch branch
# /home/peymanc/Desktop/proj/Gut_GDNF/human_Branch AB
```
