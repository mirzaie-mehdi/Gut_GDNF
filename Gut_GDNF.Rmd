...
ump_human<-data.frame(ump[rownames(human_sec_trim),])
################
## mice
...
ump <- ump_mice
## cluster numbers
clu.n  <- seq(10, 150, length.out = 15)
################################
out.s <- NULL
old.name <- NULL
for (c.n in clu.n){
...
write.table(out.s, "data/clastering/MiceAB_150clustered.txt", quote=F, sep="\t")
################################
# summary table for the cluster results
################################
out.s <- read.delim('data/clastering/MiceAB_150clustered.txt')
dat   <- as.matrix(out.s)
##########
...
write.table(out.c, "data/clastering/MiceAB_150cluID.txt", quote=F, sep="\t")
write.table(out.n, "data/clastering/MiceAB_150cluNum.txt", quote=F, sep="\t")
################################
rm(list=setdiff(ls(), sel_var))
```
...
##################
mydat <- as.data.frame(t(mice_e15[,-c(13,14)]))
myclu.s <- data.frame(out.s)
############
#myclu.s <- myclu.s[rownames(at2[grep('Branch A',at2$Integrated_05),]),]
#########
...
saveRDS(exClu,'data/rds/MiceAB_150exClu.rds')
#######################
rm(list=setdiff(ls(), sel_var))
