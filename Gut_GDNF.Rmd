...
ump <- ump_mice
## cluster numbers
clu.n  <- seq(100, 250, length.out = 16)
################################
out.s <- NULL
old.name <- NULL
for (c.n in clu.n){
...
write.table(out.s, "data/clastering/MiceAB_90clustered.txt", quote=F, sep="\t")
################################
# summary table for the cluster results
################################
out.s <- read.delim('data/clastering/MiceAB_90clustered.txt')
dat   <- as.matrix(out.s)
##########
